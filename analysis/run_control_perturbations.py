...
# Expression-matched controls (12 genes in Geneformer vocab)
CONTROL_GENE_MAP = {
...
def main():
    ...
    for i, (symbol, ensembl_id) in enumerate(CONTROL_GENE_MAP.items()):
        ...
        if existing:
            logger.info(f"  SKIP {symbol} - already has results: {existing}")
    ...
    # Save summary
    summary_file = os.path.join(OUTPUT_BASE, "control_perturbation_summary.json")
